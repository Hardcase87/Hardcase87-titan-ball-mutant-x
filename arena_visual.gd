...
func _build_the_pit() -> void:
    ...
    _box("NearRailL", Vector3(0,0.55,-9.58), Vector3(34.5,0.22,0.22), pink)
    _box("NearRailR", Vector3(0,0.55,9.58), Vector3(34.5,0.22,0.22), pink)
    _box("FarRail", Vector3(17.25,0.65,0), Vector3(0.22,0.24,19.4), acid)

    # Small field-side industrial structures only. No giant purple grandstands.
    for side_value in [-1,1]:
        var side = float(side_value)
        var z = 10.45 * side
        _box("Wall_%d" % side_value, Vector3(1.0,1.0,z), Vector3(32.0,1.7,0.75), metal)
    ...
    _cylinder("GoalStem", Vector3(16.15,1.65,0), 0.06,3.1, cyan)
    _box("GoalCross", Vector3(16.15,2.8,0), Vector3(0.10,0.10,3.6), cyan)

    _label("SKULL JUICE", Vector3(-8.0,2.0,-10.1), Vector3(0,0,0), Color(0.55,1,0.08,1), 38)
    _label("MUTANT LOOPS", Vector3(7.0,2.0,-10.1), Vector3(0,0,0), Color(1,0.05,0.55,1), 38)
    ...
    if backdrop != null:
        ...
        bg.texture = backdrop
        bg.position = Vector3(17.65,7.25,0)
    ...
    for side_value in [-1,1]:
        ...
        l.position = Vector3(4.0,2.6,10.0*float(side_value))
